...
impl InstructionSet {
    pub fn new(mmu: Rc<RefCell<Mmu>>) -> InstructionSet {
        ...
        instructions_8bit[0x07] = Instruction{ name: String::from("RLCA"), opcode: 0x07, length: 1, cycles: 1,
            operation: Operation::Nullary(Rc::new(|mmu: &mut Mmu, registers: &mut Registers| { Self::rlc(mmu, registers, |_, r| r.a_ref()); registers.set_zero_flag(false); })) } ;
        ...
        instructions_8bit[0x17] = Instruction{ name: String::from("RLA"), opcode: 0x17, length: 1, cycles: 1,
            operation: Operation::Nullary(Rc::new(|mmu: &mut Mmu, registers: &mut Registers| { Self::rl(mmu, registers, |_, r| r.a_ref()); registers.set_zero_flag(false); })) } ;
        ...
        instructions_8bit[0x1F] = Instruction{ name: String::from("RRA"), opcode: 0x1F, length: 1, cycles: 1,
            operation: Operation::Nullary(Rc::new(|mmu: &mut Mmu, registers: &mut Registers| { Self::rr(mmu, registers, |_, r| r.a_ref()); registers.set_zero_flag(false); })) } ;
        ...
        instructions_8bit[0xCF] = Instruction{ name: String::from("RST 1"), opcode: 0xC7, length: 1, cycles: 4,
            operation: Operation::Nullary(Rc::new(|mmu: &mut Mmu, registers: &mut Registers| { Self::call(mmu, registers, 0x08, 0) })) };
        ...
        instructions_8bit[0xDF] = Instruction{ name: String::from("RST 3"), opcode: 0xDF, length: 1, cycles: 4,
            operation: Operation::Nullary(Rc::new(|mmu: &mut Mmu, registers: &mut Registers| { Self::call(mmu, registers, 0x18, 0) })) };
        ...
        instructions_8bit[0xE7] = Instruction{ name: String::from("RST 4"), opcode: 0xE7, length: 1, cycles: 4,
            operation: Operation::Nullary(Rc::new(|mmu: &mut Mmu, registers: &mut Registers| { Self::call(mmu, registers, 0x20, 0) })) };
        ...
        instructions_8bit[0xEF] = Instruction{ name: String::from("RST 5"), opcode: 0xEF, length: 1, cycles: 4,
            operation: Operation::Nullary(Rc::new(|mmu: &mut Mmu, registers: &mut Registers| { Self::call(mmu, registers, 0x28, 0) })) };
        ...
        instructions_8bit[0xF7] = Instruction{ name: String::from("RST 6"), opcode: 0xF7, length: 1, cycles: 4,
            operation: Operation::Nullary(Rc::new(|mmu: &mut Mmu, registers: &mut Registers| { Self::call(mmu, registers, 0x30, 0) })) };
        ...
        instructions_8bit[0xFF] = Instruction{ name: String::from("RST 7"), opcode: 0xFF, length: 1, cycles: 4,
            operation: Operation::Nullary(Rc::new(|mmu: &mut Mmu, registers: &mut Registers| { Self::call(mmu, registers, 0x38, 0) })) };
        ...
        instructions_16bit[0x26] = Instruction{ name: String::from("SLA (HL)"), opcode: 0x26, length: 2, cycles: 2,
            operation: Operation::Nullary(Rc::new(|mmu: &mut Mmu, registers: &mut Registers| { Self::sl(mmu, registers, |m, r| m.get_byte_ref(r.hl() as usize).unwrap()) })) };
        ...
        instructions_16bit[0x2E] = Instruction{ name: String::from("SRA (HL)"), opcode: 0x2E, length: 2, cycles: 2,
            operation: Operation::Nullary(Rc::new(|mmu: &mut Mmu, registers: &mut Registers| { Self::sr(mmu, registers, |m, r| m.get_byte_ref(r.hl() as usize).unwrap()) })) };
    ...
    // Flags: Z 0 0 A7
    fn rl<F>(mmu: &mut Mmu, registers: &mut Registers, mut get_value: F) where for<'a> F: FnMut(&'a mut Mmu, &'a mut Registers) -> &'a mut u8 {
    ...
    // Flags: Z 0 0 A0
    fn rr<F>(mmu: &mut Mmu, registers: &mut Registers, mut get_value: F) where for<'a> F: FnMut(&'a mut Mmu, &'a mut Registers) -> &'a mut u8 {
        let carry_flag = registers.carry_flag() as u8;
    ...
    // Flags: Z 0 0 A7
    fn rlc<F>(mmu: &mut Mmu, registers: &mut Registers, mut get_value: F) where for<'a> F: FnMut(&'a mut Mmu, &'a mut Registers) -> &'a mut u8 {
        let value = get_value(mmu, registers);
        let carry_bit = *value & (1 << 7) != 0;
    ...
    // Flags: Z 0 0 A0
    fn rrc<F>(mmu: &mut Mmu, registers: &mut Registers, mut get_value: F)
    where
    ...
    // Flags: Z 0 0 A0
    fn sr<F>(mmu: &mut Mmu, registers: &mut Registers, mut get_value: F)
    where
        for<'a> F: FnMut(&'a mut Mmu, &'a mut Registers) -> &'a mut u8,
    {
        let value = get_value(mmu, registers);
    ...
    // Flags: Z 0 0 A0
    fn sl<F>(mmu: &mut Mmu, registers: &mut Registers, mut get_value: F)
    where
        for<'a> F: FnMut(&'a mut Mmu, &'a mut Registers) -> &'a mut u8,
    {
        let value = get_value(mmu, registers);
